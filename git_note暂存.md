...
使用
- git add
命令，可以使得工作区的所有文件，添加进暂存区。我们可以把这个过程想象成把一些物品放进打包盒中。暂存区的内容我们也是可以更改的。就好比我们又想把物品放入打包盒，或者拿出来，都是可以做到的。
- git commit
命令就相当于是把打包盒封装，寄出。此时已经属于无法修改的状态了，寄出的目的地就是本地库。就像集中式版本控制系统一样，本地库会记录下此时上传的整个打包盒的数据，成为一个历史版本。这就是一次版本控制。
...
commit之后发现日志会显示此次提交与上一次提交相比有哪些更新；这表示，这次提交并非孤立，而是与上一次提交有着某种联系，也可以称上一次提交为父提交
### 文件提交本地库
...
# 6.Github操作
对于Github，初学者或者一般用户最重要也是最常用的就是克隆远程库，推送到远程库，拉去远程库这几个功能。
而在Github平台上进行的种种操作，又涉及到权限的问题。
要使用Github第一步就是拥有一个Github账号，并且记住账号密码邮箱。
下一步就需要创建一个远程库，这又设计到一些选项，暂时不用管，要注意这个库是否开源。
创建库以后，就可以通过一些命令使用远程库。
Github也叫做Github代码托管中心，使用Github的一般都是为了把项目托管到云端库。就要使用推送命令

- git push URL链接/别名 分支
注意push的最小单位是分支。也就是说，一次push的内容，不仅仅是当前的版本，还有到达当前版本所历经的版本。

同时因为git push命令是直接会对远程库内容进行操作的，所以需要操作远程库的权限，所以push时需要github账号密码，或者密钥。

push操作会登录github账号，如果push到别人的库，就要看你的账号有没有这个权限。拉取一个账号到此项目的团队中，就可以赋予这个账号使用库的权限。![alt text](image-110.png)

假设远程库的历史版本是A-B-C，本地库当前分支的历史版本是A-B-C-D-E，push这条分支就意味着版本E和到达此版本必须的版本D也会跟着上传，最后云端版本就也成了A-B-C-D-E。
而当云端版本是这样的A-B-C-J-K,而本地版本是A-B-C-D-E，一般无法提交。

如果不想让远程库有分支，就可以在push本地分支时，使用一下
- git pull URL/别名 分支名
拉取远程库指定分支到本地。拉取操作同样视为一次版本更新。使用了拉取操作之后，就相当于更新了一下本地库的代码，使得与远程库同步。

Q: 但是比如刚刚的例子，本地库：A-B-C-D-E，远程库：A-B-C-J-K；拉取远程库到本地是会新建一个分支存放A-B-C-J-K，还是会自动合并成A-B-C-?-?,或者提示让我们手动合并？
A: 根据测试结果，类似于分支合并时出现冲突的情况，文件内容出现冲突时需要手动合并。
- git clone 远程库URL 目录路径

首先得说说git clone这个命令，
首先它操作的目录是可以自定义的，也就意味着如果想让它访问一些奇怪的位置，可能会遇到权限问题；还有一点是如果不自定义路径，就会在Git当前目录下操作。

远程库地址，可以在Github等远程库网站中库页面找到，这个地址应该是叫做URL，分为两种，一种是HTTP协议，一种是SSH协议的。

git clone这个命令本身，就是在指定目录创建一个文件夹，这个文件夹包含有当前版本的内容和.git，所以clone会创建一个新的本地库。还有一点是，clone操作视为一次版本更新。

clone操作会自动创建一个别名，内容就是clone的远程库的URL。

所以使用git clone命令会做3件事：
...
对开源项目进行clone操作是不需要登录的，因为不会对远程库进行修改。

clone和pull最大的区别在于会不会创建新的本地库。

- git clone --branch 分支名 远程库URL 
就可以指定要克隆的分支。


测试1：不同路线本地库push到远程库是会出现新分支，还是合并到一条分支上？
1. 创建一个本地库，一个远程库，把本地库push到远程库。
2. 再创建两个文件夹，克隆远程库。
3. 两个克隆库进行不同的操作，比如同一个文件进行不同修改，创建不同文件。
4. 两个克隆库分别提交远程库。

clone1提交：![alt text](image-105.png)
clone2提交：![alt text](image-106.png)

结果说明，
不会产生新的分支。

而且也不会合并到一条分支上，而是建议我先使用git pull更新一下。所以就可以进行测试2

测试2：
即当远程库与本地库的历史版本产生分歧时，结果会如何？
1. 在clone2用git pull更新。

pull： ![alt text](image-107.png)
