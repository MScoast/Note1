...
而当云端版本是这样的A-B-C-J-K,而本地版本是A-B-C-D-E，直接提交就会导致远程库出现分支：A-B-C-J-K和A-B-C-D-E（通过ai了解的，未检验）
同时因为git push命令是直接会对远程库内容进行操作的，所以需要操作远程库的权限，所以push时需要github账号密码，或者密钥。
如果不想让远程库有分支（分支是正常情况，git的强大之处也在于能管理这些分支，AI），就可以在push本地分支时，使用一下
- git pull URL/别名 分支名
...
git clone这个命令本身，就是在指定目录创建一个库，这个库包含有云端库当前分支的历史版本还有当前版本的内容。所以在已经存在库的目录下应该是不能使用git clone的。还有一点是，既然git clone的目标是按照分支来划分，是不是说明可以支持指定某个分支clone。
